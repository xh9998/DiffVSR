...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.msatten:
            ...
            nn.init.zeros_(self.attn_temporal_small.to_out[0].weight.data)

            self.temporal_down_conv = nn.Conv2d(dim, dim, kernel_size=3, stride=2, padding=1)
            self.temporal_up_conv = nn.ConvTranspose2d(dim, dim, kernel_size=4, stride=2, padding=1)

            self.norm_temporal_small = AdaLayerNorm(dim,
                                                    num_embeds_ada_norm) if self.use_ada_layer_norm else nn.LayerNorm(dim)
    ...
    def forward(self,
        ...
        if self.only_cross_attention:  # Cross-Attention
            hidden_states = (self.attn1(norm_hidden_states, encoder_hidden_states, attention_mask=attention_mask) +
                             hidden_states)
        else:  # Self-Attention
        ...
        if self.attn2 is not None:
            ...
            if self.msatten:
                ...
                hidden_states = rearrange(hidden_states, "(b f) (h w) c -> (b h w) f c", f=video_length, h=h,
                                        w=w).contiguous()
                ...
                small_hidden_states = rearrange(small_hidden_states,
                                                '(b f) c h w -> (b h w) f c',
                                                f=video_length,
                                                h=new_h,
                                                w=new_w)
...
class TemporalAttention(CrossAttention):
    ...
    def __init__(self,
        ...
        self.time_rel_pos_bias = RelativePositionBias(
            heads=heads, max_distance=32)  # realistically will not be able to generate that many frames of video... yet
