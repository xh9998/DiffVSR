...
class TemporalModule3D(nn.Module):
    def __init__(
        ...
        if self.video_condition:
            ...
            # self.v_cond_conv = InflatedConv3d(in_channels=3, out_channels=video_condition_dim, kernel_size=3, stride=1, padding=1)
        ...
        if not (attention_block_types[0]=='' and attention_block_types[1]==''):
            attentions = TemporalTransformer3DModel(
                ...
                # attention_head_dim=in_channels // num_attention_head,
        ...
        if use_scale_shift:
            # self.scale_shift_conv = zero_module(InflatedConv3d(in_channels=in_channels, out_channels=in_channels * 2, kernel_size=3, stride=1, padding=1))
...
class TemporalTransformerBlock(nn.Module):
    def __init__(
        ...
        # 4. Cross-Attn (inject context and timestep) # Remove Cross-Attn in VSR
        ...
        #     upcast_attention=upcast_attention,
        # )
        # self.norm4 = AdaLayerNorm(dim, num_embeds_ada_norm) if self.use_ada_layer_norm else nn.LayerNorm(dim)
    ...
    def set_use_memory_efficient_attention_xformers(self, use_memory_efficient_attention_xformers: bool, attention_op: None):
        if not is_xformers_available():
            ...
            # self.attn_temporal._use_memory_efficient_attention_xformers = use_memory_efficient_attention_xformers # raise error
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None):
        ...
        # hidden_states = self.attn_cross(norm_hidden_states, encoder_hidden_states=encoder_hidden_states, attention_mask=attention_mask) + hidden_states
...
class AdaLayerNorm(nn.Module):
    ...
    def forward(self, x, timestep):
        ...
        # timestep shape: b
        # pdb.set_trace()
        # if len(timestep.shape) == 0: timestep = timestep.unsqueeze(0) # fix bug during inference, when input timestep has no shape
