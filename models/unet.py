...
class UNet3DVSRModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        down_block_types: Tuple[str] = ("DownBlock3D", "CrossAttnDownBlock3D", "CrossAttnDownBlock3D",
                                        "CrossAttnDownBlock3D"),
        mid_block_type: str = "UNetMidBlock3DCrossAttn",
        up_block_types: Tuple[str] = ("CrossAttnUpBlock3D", "CrossAttnUpBlock3D", "CrossAttnUpBlock3D", "UpBlock3D"),
        ...
        self.msatten = msatten

        self.sample_size = sample_size
        time_embed_dim = block_out_channels[0] * 4

        self.downsample_2x = downsample_2x
        if downsample_2x:
            self.conv_down_2x_temporal = InflatedConv3d(3, 3, kernel_size=5, stride=2, padding=2)
    ...
    def forward(
        ...
        # treatment for latent upscaled model
        # 2x downsampling
        if self.downsample_2x:
            # residual = low_res.clone()
            # b = low_res.shape[0]
            # residual = rearrange(residual, 'b c t h w -> (b t) c h w').contiguous()
            # residual = F.interpolate(residual, scale_factor=0.5, mode='area')
            # residual = rearrange(residual, '(b t) c h w -> b c t h w', b=b).contiguous()
            # low_res = self.conv_down_2x_temporal(low_res) + residual

            low_res = self.conv_down_2x_temporal(low_res)
    ...
    def from_pretrained_2d(cls, config_path, pretrained_model_path):
        ...
        # config["num_class_embeds"] = None # Originally, num_class_embeds=1000,
        # stands for of noise level added on lr
        ...
        if freeze_pretrained_2d_upsampler:
            ...
            for k, v in model.named_parameters():
                if not 'temporal' in k:
                    ...
                    logger.info(f'trainable: {k}')

        if 'resnets' in k or 'conv_in' in k:
